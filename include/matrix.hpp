...
public:
  ...
  friend std::ostream &operator<<(std::ostream &t_out,
                                  const S21Matrix &t_mat) noexcept;
  ...
  S21Matrix operator*(double t_num);
...
public:
  ...
  void MulNumber(double t_num);

  void SetRows(std::int32_t t_rows);

  void SetCols(std::int32_t t_cols);
...
private:
  ...
  template <typename T>
  constexpr void
  Initialize(T &&t_matrix) noexcept(!std::is_lvalue_reference_v<T>);

  template <class T, class Fn>
  constexpr static void Cycle(std::int32_t &t_i, std::int32_t &t_j,
                              T &&t_matrix, Fn func);
