...
S21Matrix operator+(const S21Matrix& t_a, const S21Matrix& t_b) {
    ...
    std::transform(t_a.m_matrix, t_a.m_matrix + t_a.Size(), t_b.m_matrix, tmp.m_matrix,
                   std::plus());
    return tmp;
}
...
S21Matrix operator*(const S21Matrix& t_a, const S21Matrix& t_b) {
    if (t_a.m_cols != t_b.m_rows) {
        throw std::logic_error("the number of columns of the first matrix is not equal to the "
...
void S21Matrix::SubMatrix(const S21Matrix& t_matrix) { *this -= t_matrix; }
...
void S21Matrix::Cofactor(const S21Matrix& t_src, S21Matrix& t_dst, std::int32_t t_jmp_r,
